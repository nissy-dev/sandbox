    ...
    pub fn matches(&self, n: &Box<Node>) -> bool {
        match self {
            ...
            SimpleSelector::TypeSelector { tag_name } => {
                if let NodeType::Element(node_value) = &n.node_type {
                    return tag_name == &node_value.tag_name;
                } else {
                    return false;
                }
            }
                    ...
                    return tag_name == &node_value.tag_name
                        && (match op {
                            AttributeSelectorOp::Eq => {
                                return node_value.attributes.get(attribute) == Some(value)
                            }
            ...
            SimpleSelector::ClassSelector { class_name } => {
                if let NodeType::Element(node_value) = &n.node_type {
                    return node_value.attributes.get("class") == Some(class_name);
                } else {
                    return false;
                }
            }
